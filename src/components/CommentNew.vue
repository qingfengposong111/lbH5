<template>
  <div>
    <v-scroll style="height: 100%;background: #fff;" :on-refresh="onRefresh" :on-infinite="onInfinite">
      <router-link  v-for="(temp,index11) in listdata" :key="'a'+index11" :to="'/home/activity/userCommentDetail/'+temp.id" class="comment">
        <div style="display:flex;">
          ...
          <div class="com-box">
            ...
            <div>
              <router-link to='' v-if="len1>100" class="com-all"  @click.native="viewAll(index11)">{{temp.hole}}</router-link>
              <div class="pic-box">
                <flexbox :gutter='12.5' style="width: 33%;float: left;margin-bottom:3px;"   v-for='(item,s) in temp.urls' :key="s">
      ...
      <router-link v-for="(child,index3) in downdata" :key="index3"  :to="'/home/activity/userCommentDetail/'+child.id" class="comment">
        <div style="display:flex;">
          ...
          <div class="com-box">
            ...
            <div>
              <router-link to='' v-if="len2>10" class="com-all" @click.native="viewAll2(index3)">{{child.hole}}</router-link>
              <div class="pic-box">
                <flexbox :gutter='12.5' style="width: 33%;float: left;margin-bottom:3px;"   v-for='(item,r) in child.urls' :key="r" >
                  <flexbox-item   class='find-main-f'>
                    <div class="com-pic">
                      <img :src="item" alt="">
                    </div>
        ...
        this.$axios.get('/api/v2/activityComments',{
          ...
          .then(res =>{
            ...
            for(; i<end; i++){
              ...
              for(var j=0;j<this.downdata.length;j++){
