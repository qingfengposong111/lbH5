    ...
    <div style="padding-top:44px;">
      <router-link  to='' class="comment">
        ...
        <div class="com-box">
          ...
          <div>
            <router-link to='' class="com-all" @click.native="fullless">{{hole}}</router-link>
            <div class="pic-box">
        ...
        <div class="commentBox" >
          ...
          <div v-else>
            <div class="reply">
              ...
              <p v-if="temp.replyMid!=null"  v-for="(temp,index4) in commList" v-model="commList" :key="'a'+index4"><span class="autor" >{{temp.nickname}}</span> 回复 <span @click="changeCommenter"  class="autor">{{temp.replyNickname}}：</span><span>{{temp.evaluation}}</span></p>
            </div>
          </div>
        </div>
      </div>
...
<script>
  export default {
    ...
    computed:{},
    created(){
      this.$axios.get('/api/v2/schoolCommentDetail1',{
        params:{
      ...
      this.$axios.get('/api/v2/schoolCommentDetail2',{
        params:{
          id: this.$route.params.id
        }
      })
    ...
    methods:{
      ...
      blur ( ) {
        ...
        if(this.type === 0) {
          this.$axios.post('/api/v2/schoolReply',{
            ...
            .catch(err =>{
             /* this.$vux.loading.show({
              ...
              setTimeout(() => {
                ...
                that.$router.push({name:'login',query:{redirect:'/school/schoolComDetail/'+that.$route.params.id}});
              }, 1000)*/
      ...
      write () {
        let that = this;
        if(localStorage.token){
          this.$axios.post('/api/v2/schoolLike',{
