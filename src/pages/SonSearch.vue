<template>
  <div class='home-act'>
    ...
    <div class="act-main">
      <router-link :to="'/home/activity/detail/'+item.id" :key="index" class="act-box" v-for="(item,index) in getList">
        <div class="act-banner">
          <img :src="item.actImg" alt="">
        </div>
        <flexbox :gutter="3">
          <flexbox-item :span='11/13'> <h4 class="act-title">{{item.activeName}}</h4></flexbox-item>
          <flexbox-item :span='2/13'><div class="act-status" :class="{'act-end': item.isEnd===3}"><p>
            <span v-if="item.isEnd===1">报名中</span>
            <span v-if="item.isEnd===2">进行中</span>
            <span v-if="item.isEnd===3">已结束</span>
          </p></div></flexbox-item>
        </flexbox>
        <div class="act-foot">
          <flexbox>
    ...
    methods: {
          ...
          .then(res => {
            for(let i=0;i<res.data.data.list.length;i++){
              if(res.data.data.list[i].activeName.trim().indexOf(this.msg)>=0){
                this.ars.push(res.data.data.list[i]);
              }
            }
            this.$store.commit('set_list', this.ars);//将值传入store面
          })
          .catch(err => {
          })
      },
      haveInHand(isEnd) {
        ...
        this.$axios.get('/api/v2/findYxwActivityList')
          .then(res => {
            for(let i=0;i<res.data.data.list.length;i++){
              if(res.data.data.list[i].activeName.trim().indexOf(this.msg)>=0&&res.data.data.list[i].isEnd===1){
                this.ars.push(res.data.data.list[i]);
              }
            }
            this.$store.commit('set_list', this.ars);//将值传入store面
      ...
      over() {
        ...
        this.$axios.get('/api/v2/findYxwActivityList')
          .then(res => {
            for(let i=0;i<res.data.data.list.length;i++){
              if(res.data.data.list[i].activeName.trim().indexOf(this.msg)>=0&&res.data.data.list[i].isEnd===3){
                this.ars.push(res.data.data.list[i]);
              }
            }
            this.$store.commit('set_list', this.ars);//将值传入store面
