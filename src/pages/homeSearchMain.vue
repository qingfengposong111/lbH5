  ...
  <div class='home-act' >
    ...
    <div class="tabs">
      <tab :line-width="1" custom-bar-width="100px"  @on-before-index-change="switchTabItem">
        <tab-item selected >
          <router-link class="tab-act" :to="'/home/homeSearchMain/TK'"  @click.native='getData()'>{{hole}}<i class='choose-icon'></i>
          </router-link>
        </tab-item>
    ...
    <mescroll-vue ref="mescroll" class="mescroll" :down="mescrollDown" :up="mescrollUp" @init="mescrollInit">
      <div>
    <c-panel style='margin-top:88px;' :list="getScreen" @on-img-error="onImgError"></c-panel>
      </div>
    </mescroll-vue>
  ...
  export default {
    ...
    methods: {
      ...
      getData: function() {
        ...
        this.$axios.get('/api/v2/search', { params: param }).then(res => {
      ...
      upCallback (page, mescroll) {
          this.$axios.get('/api/v2/search', {// 请求的列表数据
            params: {
              pageSize: page.size,
              pageNumber: page.num,
              town: this.$store.state.searchDist||'',
